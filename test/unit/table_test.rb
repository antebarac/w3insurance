...
DELIMITER
  ...
  def test_crazy_spacing
    ...
    assert_equal BigDecimal.new(0.0084, 4), t.evaluate(:osigurana_svota => 120000)[:stopa]
  end
  
  def test_bool
    t = Table.new(@@definicija_bool)
    assert_equal BigDecimal.new(0.0084, 4), t.evaluate(:osigurana_svota => true)[:stopa]
  end
